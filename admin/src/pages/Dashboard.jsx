...
export default function Dashboard() {
  const [stats, setStats] = useState({ products: 0, forms: 0, orders: 0, visitors: 0 });

  useEffect(() => {
    async function load() {
      const since = new Date(Date.now() - 5 * 60 * 1000).toISOString();
      const [products, forms, orders, visitors] = await Promise.all([
  ...
  return (
    <div className="page">
      <header className="page-head">
        <h1>Dashboard</h1>
        <p>Catalog, forms, orders, and live visitors.</p>
      </header>
